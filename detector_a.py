...
def main():
  ...
  for dev in devlist:
    ...
    # Log
    sql = "INSERT INTO history (id, name, type, scantime, sensor) VALUES (%s, %s, %s, %s, 'a')"
    c.execute(sql, (mac, name, type, scantime)) 

    # Devices
    sql = "INSERT INTO devices (id, name, service, major, minor, lastseen, a_seen) VALUES (%s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE name = VALUES(name), service = VALUES(service), major = VALUES(major), minor = VALUES(minor), lastseen = VALUES(lastseen), a_seen = VALUES(a_seen)"
